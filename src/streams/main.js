...
function createContentWindow({ x, y, width, height, monitorId, contentType, path: contentPath, loop = false }) {
  ...
  const display = displays[monitorId];
  const scaleFactor = display.scaleFactor || 1;
  const adjustedWidth = Math.round(width);
  const adjustedHeight = Math.round(height);
  const adjustedX = Math.round(x);
  const adjustedY = Math.round(y);

  const win = new BrowserWindow({
    x: adjustedX + display.bounds.x, y: adjustedY + display.bounds.y, width: adjustedWidth, height: adjustedHeight,
    frame: false, transparent: true, alwaysOnTop: true,
    webPreferences: { 
      nodeIntegration: true, 
      contextIsolation: false, 
  ...
  const windowInfo = { id: win.id, monitorId, contentType, path: contentPath, x, y, width: adjustedWidth, height: adjustedHeight, loop };
